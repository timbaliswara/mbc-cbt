...
<section class="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
    <div class="rounded-md border border-zinc-200 bg-white p-6 text-center shadow-sm">
        <p class="text-sm font-medium uppercase tracking-[0.16em] text-emerald-700">Ujian selesai</p>
        ...
        @if ($attempt->exam->show_result_to_student && $attempt->result)
            ...
            @if ($attempt->result->essay_status === 'pending')
                <p class="mt-5 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">Nilai esai masih menunggu koreksi admin.</p>
            @endif
        @else
            <p class="mt-6 rounded-md border border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-600">Jawaban sudah masuk. Hasil akan diinformasikan oleh admin MBC.</p>
        @endif
