...
new class extends Component
{
    ...
    {
        $this->attempt = $attempt->load(['exam.questions', 'student', 'result']);
    }
};
?>

<section class="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
    @php($maxScore = ExamScoring::maxScore($attempt))
    <div class="rounded-md border border-zinc-200 bg-white p-6 text-center shadow-sm">
        ...
        @if ($attempt->exam->show_result_to_student && $attempt->result)
            <div class="mt-8 grid gap-4 sm:grid-cols-3">
                ...
                <div class="rounded-md border border-zinc-200 p-4"><p class="text-sm text-zinc-500">Nilai</p><p class="mt-2 text-2xl font-semibold text-emerald-700">{{ $attempt->result->total_score }}</p><p class="mt-1 text-xs text-zinc-500">dari {{ $maxScore }}</p></div>
            </div>
